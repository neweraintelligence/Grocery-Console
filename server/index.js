...
app.get('/api/shopping-list', async (req, res) => {
    try {
        ...
        const shoppingItems = rows
            ...
            const onList = onListValue && (onListValue.toString().toUpperCase() === 'TRUE' ||
                onListValue === true ||
                onListValue === 1 ||
                onListValue === '1' ||
...
app.get('/api/recipes', async (req, res) => {
    try {
        ...
        const suggestedRecipes = recipeDatabase
            .map(recipe => {
            const matchingIngredients = recipe.ingredients.filter(ingredient => availableItems.some((item) => item.includes(ingredient.toLowerCase()) ||
